...
# NVIDIA CUDA Toolkit (gives nvcc). Large download (~3 GB).
Install 'Nvidia.CUDA'    'cuda'     'CUDA Toolkit 12.x (nvcc)'
...
Write-Host "  nvcc --version        (CUDA compiler)"
Write-Host "  nvidia-smi            (driver / GPU)"
Write-Host "  cl                    (run inside 'x64 Native Tools Command Prompt for VS 2022')"
Write-Host "`nNotes:" -ForegroundColor Yellow
Write-Host " - CUDA needs a recent NVIDIA driver; if nvidia-smi is old, update GeForce/Studio driver."
Write-Host " - For CUDA builds, ALWAYS build from the 'x64 Native Tools Command Prompt for VS 2022'"
