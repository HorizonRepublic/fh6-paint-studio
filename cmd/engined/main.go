...
func main() {
	...
	// Accepted and ignored. The service now lives and dies with the pipes its parent holds, so there
	// is nothing left for a timeout to rescue — but a client built against the old binary still
	// passes it, and rejecting it would send usage to stderr and leave that client with a silent
	// stdout and no explanation.
	_ = flag.Duration("idle-timeout", 0, "accepted and ignored; the service ends when its parent does")
	flag.Parse()
	...
	err := enginesvc.Serve(enginesvc.Options{LibraryRoot: *lib})
	if err != nil {
