...
func main() {
	...
	region := flag.String("region", "", "reconstruct only a sub-region as its OWN decal: fx,fy,fw,fh (fractions of the source, 0..1). The crop is taken at FULL source resolution THEN scaled to -max-res, so the region fills the render вЂ” a face crop gets far more shapes/pixels than the same region inside the whole-image budget. The path to crisp detail: the detail's angular size in the decal sets its crispness, so a face inside a whole-scene decal is small and soft. Output coords are in the crop's own space (a standalone decal).")
	gridSize := flag.Int("grid", 48, "error-grid resolution (NxN cells) for importance sampling; finer (96-160) targets shapes onto small high-contrast features (thin contours, text) instead of spreading them вЂ” helps fine detail at the cost of a slightly more scattered coarse stage")
	...
	overdraw := flag.Float64("overdraw", 1.0, "generate -shapes*overdraw, keep the most useful -shapes (>1 trades time for a small quality-per-shape gain; opaque-only вЂ” ignored with -alpha)")
	alpha := flag.Bool("alpha", true, "allow semi-transparent shapes (alpha<1) on opaque images вЂ” many soft layers build smooth gradients/fur in fewer shapes (auto-OFF for cutouts so the object stays opaque)")
	...
	standout := flag.Float64("standout", 0, "post-polish PERCEPTUAL standout suppression: detect shapes whose rim draws an edge the TARGET lacks (a visible circle/square the SSE metric is blind to) and recolour-to-local-mean or remove them, gated so the GLOBAL error rises at most this fraction. 0=off. ~0.005 = conservative. The metric will NOT show the win вЂ” judge by eye; the gate only bounds the loss.")
	...
	shapeTol := flag.Float64("shape-tol", 0, "auto-shape-count: stop placing shapes when the relative marginal improvement rate r=О”Err/(windowВ·currentErr) per shape stays below this (0=off, fill -shapes budget). Adapts the count per image: saturated flat/logo stop early (~175-400), detailed photo/anime/cartoon fill the budget. -shapes is the ceiling. Recommended auto value: 0.0002 (conservative; trims only genuinely-saturated content). 0.0005 = aggressive/draft.")
	kneeFloor := flag.Float64("knee-floor", 0, "with -shape-tol: floor the knee denominator at this fraction of the INITIAL error so the same tol also trips on near-SOLVED content (clean line-art / fully-filled flats) where Г·currentErr blows up and never stops. 0=off (pure relative). ~0.02 = treat <2% residual as solved. Detailed photos (currentErr в‰« floor) are unaffected.")
	minGain := flag.Float64("min-gain", 0, "low-contrast shape GATE: reject a shape whose mean per-pixel SSE improvement (в€’score/area) is below this вЂ” a faint 'ghost facet' that barely differs from what it covers. Budget reallocates to real detail or auto-stops once nothing high-contrast remains. 0=off. The direct fix for flat-background over-fill. Tune by EYE (too high erodes soft gradients). Working space is linear-light 0..1 RGBA, so per-pixel SSE is small вЂ” try ~1e-4..1e-3.")
	...
	ssaa := flag.Int("ss", 1, "preview supersampling factor (1=off): render the output shapes at ssГ— then box-downsample for ANTI-ALIASED edges. Our raster uses hard binary coverage, so contours are 1px steps while the source images have soft ~1-2px ramps вЂ” that mismatch is where nearly all residual image-space error sits. ss=3-4 closes it. Affects the preview/comparison render only (the game rasterizes the geometry itself).")
	gpuSearch := flag.Bool("gpu-search", true, "CUDA build only: run each shape's random-candidate phase on-device (generate+score+argmin in one launch) вЂ” the throughput unlock for high candidate volume. Ignored by the CPU backend (host path).")
	...
	momentDetailStart := flag.Float64("moment-detail-start", 0, "with -moment-seed: HYBRID schedule вЂ” past this progress (0..1) hand the per-shape search off from the moment pool to the blind random brute force, which finds the sharp SMALL detail shapes the 2nd-moment blob fit never proposes (the late shapes are cheap, so it buys crispness for little time). 0 = off (moment all the way). ~0.6-0.7 = fast smooth base + sharp random detail.")
	coarseSearch := flag.Bool("coarse-search", true, "CUDA build only: coarse-to-fine search вЂ” score the candidate batch at a CHEAP pixel cap (-coarse-budget) to filter, then re-score only the -coarse-k survivors at the full -sample-budget and pick from those. The winner is full-budget scored (quality-neutral вЂ” unlike a uniform -sample-budget cut, which mis-picks on low-res noise), while the bulk pays only the coarse cost. The dominant eval-speed lever at the quality preset (roughly halves eval wall-time). Auto-disabled below ~33k candidates (n>4*-coarse-k). -coarse-search=false for the exhaustive single-pass.")
	...
	warpEval := flag.Bool("warp-eval", false, "CUDA build only: warp-per-candidate eval kernel (opt-in). Slower than the default block-per-candidate kernel вЂ” large early shapes dominate runtime and want 128 threads/candidate, not 32. Kept for reference.")
	...
	gaussian := flag.Bool("gaussian", false, "NICHE MODE: reconstruct the image as -shapes soft GLOW splats jointly trained by the polish (no greedy, no densify) вЂ” engine.GenerateGaussian. For SMOOTH / gradient / painterly content only (8x better than greedy on a gradient; loses on fine detail). -polish-iters sets the training budget (0 = auto-scaled to the glow count). Output glows are native FH6 KindGlow primitives.")
	polishIters := flag.Int("polish-iters", 200, "polish gradient-descent iterations. Default 200 organic / 300 flat (set below) вЂ” the perceptual knee: past it the result is indistinguishable by eye even though the hard-loss metric keeps inching down, so the extra iters are wasted wall-time (polish is 60-85% of a run). Raise for max metric fidelity.")
	...
	anneal := flag.Int("anneal", 0, "EXPERIMENTAL basin-hopping / iterated local search (0=off): after greedy+polish, run N outer iterations that randomly kick (remove low-value shapes + regrow vs residual), short-re-polish, and Metropolis-accept (escaping the greedy local minimum), keeping the best. For the LOW-budget economy regime (50-300 shapes); ~20-40 iters. Costly (re-polishes each iter) вЂ” use only at low -shapes.")
	economy := flag.Bool("economy", false, "opt-in to the auto economy schedule (LIVE co-adaptation / anneal at в‰¤~1500 shapes): better quality at low budgets but ~4x slower (re-polishes all shapes per batch). Off by default. Explicit -live/-anneal override it regardless.")
	...
	imgBlur := flag.Int("img-blur", 0, "with -img-vs: box-blur the compared (-img-vs) image by this radius before diffing вЂ” tests the anti-aliasing hypothesis (does softening our hard edges match an AA target?).")
	linear := flag.Bool("linear", true, "composite in LINEAR light вЂ” the space the editor renders in (gamma ~2.2). DEFAULT-ON (matches the studio): the engine optimises the linear composite so the in-game result matches the target and semi-transparent shapes stop 'popping'; output colours are sRGB-encoded; opaque content is unaffected. Use -linear=false for an sRGB comparison. Measure with -fh6-score.")
	...
	perceptualLuma := flag.Bool("perceptual-luma", false, "EXPERIMENT (default off): compute WeightMapV2's luma in sRGB space so its darkness/highlight pivots land correctly in the linear pipeline. A/B only вЂ” validate by eye end-to-end (REVIEW M4).")
	flag.Parse()
	...
	// bumper panel is ~1000, a full side or roof is ~3000 вЂ” each panel is its own
	...
	// other and exit. Convention-free вЂ” renderer / type-id / ellipse-convention differences
	...
	// score it against the sRGB target вЂ” quantifies the semi-transparent "pop" and whether -linear fixes it.
	if *fh6Score != "" {
	...
	if *padTransparent > 0 {
		...
		applog.Printf("pad-transparent: %.2f в†’ %dx%d surround (spill-penalty bounds shapes inside the image)", *padTransparent, prep.W, prep.H)
	}
	...
	// Resolve the content MODE FIRST вЂ” it drives three things: alpha (flat/line-art ->
	...
	// Hybrid: generate the FDoG ink lines FIRST (from the source) so we know their ACTUAL count вЂ” then give
	// the fill the rest of the budget, so nothing is wasted when FDoG self-limits below its ceiling.
	// -hybrid-ink<0 = AUTO: pick the ink ceiling by content (photoв†’0, line-artв†’35%, celв†’12%, else 20%) and
	...
	// All per-mode constants come from presetpkg.ModeDefaultsFor вЂ” the SINGLE source of truth shared
	...
	// image-space SSE and SSIM, so soften it per content вЂ” flat/vector -> uniform (0), anime ->
	...
	if !userSet["weight-strength"] {
		*weightStrength = md.WeightStr // anime 0.15 / photo 0.40 / flat 0 (the only animeв‰ photo knob)
	}
	...
	// opts out (e.g. for speed вЂ” flat polish at 3000 shapes is ~half the wall time).
	if !userSet["polish"] {
		*polish = true
	}
	...
	// than the cleaner-contour benefit buys on already-clean sources, so auto = none вЂ” both are kept as
	...
	if *lockColorFlag != "" && *scoreJSON == "" && *polishJSON == "" && !*gaussian {
		...
		allowAlpha = false
		applog.Printf("mono lock-colour #%02X%02X%02X вЂ” binarized target to a single-colour cutout",
	...
	// PERCEPTUAL error, by weighting each pixel's linear-SSE by (d sRGB/d linear)ВІ. The sRGB EOTF
	// is steep in darks / flat in brights, so this up-weights shadow detail and down-weights bright
	// regions exactly as perception does вЂ” making the analytic weighted-mean optimal colour solve
	...
	if prep.HasTransparency {
		applog.Printf("transparent background detected вЂ” keeping background empty (cutout mode)")
	}
	...
	o := engine.Options{
		...
		// early-stops short of the budget вЂ” forcing small shapes fights the large flat fills
	...
	// colour/detail fill вЂ” the optimized engine renders alive eyes + smooth shading that flat cells can't,
	...
	if *preview != "" || *saveLib || *metrics {
		// WYSIWYG render: the way the GAME composites вЂ” LINEAR light вЂ” so semi-transparent shapes show
	...
	if *metrics && padPx == 0 && outW == prep.W && outH == prep.H {
		// canvas (RenderFH6) is sRGB; prep.Pixels is LINEAR in -linear mode вЂ” encode it to sRGB so the
		...
		fmt.Printf("[metrics] shapes=%d О”E76 mean=%.2f p95=%.2f  SSIM=%.4f  band=%.2f\n", len(res.Shapes)-1, de, p95, ss, band)
	}
	if *preview != "" {
		must(ensureDir(*preview))
...
// runStylize runs a stylizer preset and writes injectable geometry (+ optional preview) вЂ” the second
// pipeline's CLI entry. Coordinates are at the working resolution (src.WГ—src.H), so inject at that
