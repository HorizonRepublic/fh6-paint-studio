...
func (randomSource) search(r *run, progress float32, sampGrid []float32, penalty func(model.Candidate) float32) (model.Candidate, float32) {
	...
	t0 := time.Now()
	cands := RandomShapes(r.rng, w, h, r.opt.RandomSamples, r.kinds, r.kindWeights, r.sampler, progress, r.orient, r.coh, r.aspectCap, r.allowAlpha, r.alphaMin, r.opt.AspectMax, r.boundCtx, r.kindGate)
...
func (momentSource) search(r *run, progress float32, sampGrid []float32, penalty func(model.Candidate) float32) (model.Candidate, float32) {
	...
	for k := 0; k < seeds; k++ {
		...
		if scx, scy, srx, sry, sth, ok := momentSeedFromGrid(sampGrid, r.gw, r.gh, w, h, px, py, cR); ok {
			...
			pool = append(pool, RandomShapes(r.rng, w, h, perSeed, r.kinds, r.kindWeights, r.sampler, progress, r.orient, r.coh, r.aspectCap, r.allowAlpha, r.alphaMin, r.opt.AspectMax, r.boundCtx, r.kindGate)...)
		}
	}
