...
// deleted internal/backend/cpu when the runtime CPU backend was dropped. TEST-ONLY: an independent
// implementation of the analytic eval math, maintained without a runtime backend behind it.
//
// This is Vulkan's correctness anchor (owner decision 2026-08-03: Vulkan is the one supported
// backend, CUDA is kept unmaintained). Vulkan must NOT be validated only against CUDA: two GPU
// ports can — and did — agree while both were wrong (the gradient-flag gate, found 2026-08-03).
// A reference that shares no code with either shim is what makes a green test mean something.
// The polish reference lives separately in engine.PolishStepProbe (host Go).
